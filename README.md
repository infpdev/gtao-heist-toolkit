...
* **[AFK Jobs Key Holder](#standalone-scripts)**: Script to hold a key for AFK jobs, with customizable hotkeys and on-screen instructions
...
- **[AFK-Jobs-Key-Holder](https://github.com/infpdev/gtao-heist-toolkit/releases/tag/utilities)** — Script to hold a key for AFK jobs, with customizable hotkeys and on-screen instructions
...
> [!NOTE]
> * All standalone puzzle solvers already include built-in **NoSave** support, so the separate NoSave standalone is only needed if you want to use NoSave by itself.<br><br>
> * The **NoSave** and **AFK Jobs Key Holder** standalone downloads are not SFX packages. Simply download and run the `.exe` file to launch the script.
...
│  │
│  ├─ standalone scripts/             # Standalone scripts - Solvers, NoSave and AFK key holder
│  │
...
- `AFK-Jobs-Key-Holder-Standalone.ahk` — AFK key holder with customizable hotkeys and on-screen instructions for AFK jobs
