...
Terminate(*) {
    ...
    if (isGtaFocused() || !afkHolding)
        ExitApp
    else {
        ShowCenteredToolTip "GTA must be focused to terminate script. Focus GTA and press " . StrTitle(GetKeyName(
            terminateKeyReg)) . " again.", 1
...
UpdateTooltip(terminationStart := 0) {
    ...
    ;         . "s`nRelease the key to cancel", 0, 0
